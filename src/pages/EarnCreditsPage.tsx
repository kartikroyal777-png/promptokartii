...
const EarnCreditsPage: React.FC = () => {
  const { profile, dailyClaims, loadingProfile, claimTelegramReward, claimAdReward } = useProfile();
  const { user } = useAuth();
  ...
  const handleClaimAd = (slot: number) => {
    const isClaimed = dailyClaims.some(c => c.reward_slot === slot);
    if (isClaimed || loadingProfile) return;
  ...
  const handleAdComplete = async (success: boolean) => {
    ...
    if (success && currentSlot !== null) {
      await claimAdReward(currentSlot);
    }
    setCurrentSlot(null);
  };
  ...
  return (
    <>
      <AdOverlay isVisible={isAdVisible} onComplete={handleAdComplete} />
